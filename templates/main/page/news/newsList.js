layui.config({
			...
			layer.confirm('确定删除选中的信息？',{icon:3, title:'提示信息'},function(index){
	            ...
	            setTimeout(function(){
					...
					success: function(json){
						     // newsData.splice(i,1);
						     // window.sessionStorage.removeItem("addNews",JSON.stringify(newsData[i]));
						     // newsList(newsData);
	...
	$("body").on("click",".news_edit",function(){  //编辑
		...
		var _this = $(this);
	newsId =	_this.attr("value");
	//alert (newsId);
		var index = layui.layer.open({
			title : "文章编辑",
			type : 2,
			content : "newsEdit.html?newsId=" +  newsId,
			success : function(layero, index){
		...
		function renderDate(data,curr){
			...
			if(currData.length != 0){
				for(var i=0;i<currData.length;i++){
			    	...
			    	+'<td>'
					+  '<a class="layui-btn layui-btn-mini news_edit" value="'+data[i].newsId+'"><i class="iconfont icon-edit"></i> 编辑</a>'
