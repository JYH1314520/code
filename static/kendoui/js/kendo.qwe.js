...
!(function ($) {
    ...
    $l = function (code, value) {
        var al = arguments.length, p = Qwe.defaultPrompt;
        if (al == 1) {
            var v = p[code];
            return v ? v : code;
        } else if (al == 2) {
    ...
    if (!window.Qwe) {
         ...
         * @param v
         */
        Qwe.getCodeMeaning = function (data, v) {
        ...
        Qwe.showTip = function (msg) {
            if (!Qwe.systemNotification) {
                Qwe.systemNotification = $('<span id="systemNotification" style="display:none;"></span>').appendTo(document.body).kendoNotification({
                    stacking: "down",
                    show: function (e) {
            ...
            Qwe.systemNotification.show(msg)
        }
        /**
         * 删除选中数据(grid).
         *
         ...
         * @param opts
         */
        Qwe.deleteGridSelection = function (delSelection) {
        ...
        Qwe.onMessage = function (action, callback) {
        ...
        Qwe.showBadge = function(datas,options) {
        ...
        Qwe.blockUI = function(options) {
            ...
            if (options.animate) {
                ...
                html = '<div class="loading-message ' + (options.boxed ? 'loading-message-boxed' : '') + '"><span>&nbsp;&nbsp;' + (options.message ? options.message : $l('Qwe.loading')) + '</span></div>';
            } else {
                html = '<div class="loading-message ' + (options.boxed ? 'loading-message-boxed' : '') + '"><img src="' +  _basePath + '/lib/assets/global/img/loading-spinner-grey.gif" align=""><span>&nbsp;&nbsp;' + (options.message ? options.message : $l('Qwe.loading')) + '</span></div>';
            }
        ...
        Qwe.submitForm = function (opts) {
            ...
            $.ajax({
                ...
                complete:function () {
                    Qwe.unblockUI(shadowMask.target);
                }
            });
        };
        ...
        Qwe.autoResizeGrid = function (grid_id) {
            resizeGrid(grid_id);
         ...
         *
         * */
        Qwe.passwordFormat = function (value, temp) {
            if (temp == 'no_limit') {
                return true;
        ...
        Qwe.formatDate = function (value) {
            if (!value)return '';
            var d = (value instanceof Date) ? value : Qwe.strToDate(value);
            return kendo.toString(d, "yyyy-MM-dd")
        }
        //add by jinqin.ma@hand-china.com
        Qwe.bytestosize = function (bytes) {
        ...
        Qwe.formatDateTime = function (value) {
            if (!value)return '';
            var d = (value instanceof Date) ? value : Qwe.strToDate(value);
         ...
         * </ul>
         */
        Qwe.createAnchor = function (text, func) {
        ...
        Qwe.prepareQueryParameter = function (obj, options) {
            obj = obj || {};
            if (options) {
        ...
        Qwe.prepareSubmitParameter = function (options, type) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data.success) {
                        ...
                        Qwe.resolveError(data, options.error);
                    }
                },
                error: function (res) {
                    Qwe.resolveError(res.responseJSON)
                }
            });
        }
        ...
        $(document).ajaxSuccess(function (event, xhr, options, json) {
            ...
            // if(xhr.responseJSON)
              ...
              //  Qwe.resolveError(xhr.AssertionError)
        });
        ...
        Qwe.getDurationTime = function (durationTime) {
         ...
         * @param callback
         */
        Qwe.initEnterQuery = function (selector, callback) {
         ...
         *
         */
        Qwe.showToast = function(options){
            var opts = $.extend({
                "closeButton":true,
                "debug": false,
         ...
         * @param options
         */
        Qwe.createWindow = function(selector, options){
         ...
         * viewModel 加载数据
         */
        Qwe.loadViewModel = function(opt){
            if(opt.url){
                $.ajax({
                    url: opt.url,
         ...
         * @param extension
         */
        Qwe.createGridViewModel = function(selector, extension){
         ...
         */
        Qwe.lockBtn = function (options) {
           options = $.extend({
                "target":".Qwe-submit-load",
                "message": ""
            },options || {});
         ...
         * @param options
         */
        Qwe.unLockBtn = function (options) {
            options = $.extend({
                "target":".Qwe-submit-load",
                "message": ""
            },options || {});
         ...
         * @param options
         */
        Qwe.createGridDataSource = function(options){
                ...
                transport: {
                    ...
                    parameterMap: function (options, type) {
                        if (type !== "read" && options.models) {
                            return kendo.stringify(Qwe.prepareSubmitParameter(options, type));
                        } else if (type === "read") {
    ...
    Qwe.createGridFlexField = function (ruleSetCode, viewModel, gridId,callback) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (gridId) {
                    ...
                    Qwe.createFlexGrid(data, gridId, options);
                }
    ...
    Qwe.createFlexField = function (ruleSetCode, viewModel, divId,callback) {

        if (!viewModel._flexField) {
            viewModel._flexField = [];
        } else {
            Qwe.cleanFlexData(viewModel);
        }
        $("#" + divId).empty();
        $.ajax({
            type: "POST",
                ...
                if (data.rows) {
                    ...
                    if (divId) {
                        for (var i = 0; i < data.rows.length; i++) {
                            Qwe.createFlexRow(data.rows[i], $("#" + divId), viewModel);
                        }
    ...
    Qwe.createFlexGrid = function (data, gridId, option) {
        ...
        if (data.rows) {
            for (var i = 0; i < data.rows.length; i++) {
                for (var j = 0; j < data.rows[i].fields.length; j++) {
                    ...
                    var columnName = Qwe.flexTocalme(column.columnName);
                    ...
                    if (style.type == 'LOV') {
                        option.push({
                            ...
                            })(style.conditionFieldLovCode, style.conditionFieldSelectTf.substring(style.conditionFieldSelectTf.indexOf('.')+1), columnName),
                            headerAttributes: {
                        ...
                        option.push({
                            field: Qwe.flexTocalme(column.columnName),
                            title: style.labelName,
                            width: column.fieldColumnWidth,
                            headerAttributes: {
                                style: "text-align:center"
    ...
    Qwe.cleanFlexData = function (viewModel) {
        var str1;
    ...
    Qwe.createFlexRow = function (rows, parentDiv, model) {
        var row = Qwe.flexCe('div', parentDiv, {'class': 'row'});
        for (var i = 0; i < rows.fields.length; i++) {
            Qwe.createFlexColumn(rows.fields[i], row, model);
        }
    }
    Qwe.createFlexColumn = function (column, parentDiv, model) {
        var cloumnName = Qwe.flexTocalme(column.columnName);
        model._flexField.push(cloumnName);
        var style = $.parseJSON(column.fieldType);
        var div1 = Qwe.flexCe('div', parentDiv, {'class': 'col-xs-' + column.fieldColumnWidth});
        var form_group = Qwe.flexCe('div', div1, {'class': 'form-group'});
        var label = Qwe.flexCe('label', form_group, {
            'class': 'control-label col-xs-' + style.labelWidth,
            style: 'text-align: right'
        }).text(style.labelName);
        var div = Qwe.flexCe('div', form_group, {'class': 'col-xs-' + style.columnWidth});
    ...
    Qwe.flexCe = function (name, parent, attr) {
        var e = $('<' + name + '/>', attr)
        if (parent) {
            e.appendTo(parent);
        }
